...
export default function ReviewStep() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setIsSubmitting(false); [cite: 99]
    }
  };

  return (
    <div className="space-y-8">
      <h2 className="text-xl font-bold border-b pb-4">Review Your Application</h2>
      
      {/* Displaying information in read-only format  */}
      <section className="space-y-4">
        <div>
          <h3 className="font-semibold text-gray-700">Applicant</h3>
          <p className="text-sm">{values.fullName} ({values.kennitala})</p>
          <p className="text-sm">{values.email} | {values.phone}</p>
        </div>
